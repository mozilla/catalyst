...
class ReportGenerator:
                ...
                for metric in self.data[metric_type]:
                    ...
                    if kind == "categorical":
                        ...
                        for branch in self.data["branches"]:
                            if (
                                "uplift"
                                in self.data[branch][segment][metric_type][metric]
                            ):
                                rows = []
                                n_labels = len(
                                    self.data[branch][segment][metric_type][metric][
                                        "labels"
                                    ]
                                )
                                for i in range(n_labels):
                                    label = self.data[branch][segment][metric_type][
                                        metric
                                    ]["labels"][i]
                                    uplift = self.data[branch][segment][metric_type][
                                        metric
                                    ]["uplift"][i]

                                    # Enumerated histograms have a lot of labels, so
                                    # try and limit the ones
                                    # we show.
                                    ...
                                    if uplift > 0:
                                        uplift = "+{0:.2f}".format(
                                            self.data[branch][segment][metric_type][
                                                metric
                                            ]["uplift"][i]
                                        )
                                    else:
                        ...
                        for i in range(len(branches)):
                            total_rowspan = (
                                total_rowspan + branches[i]["branch_rowspan"]
                            )

                        categorical_metrics.append(
                            {
                                "name": metric_name,
                                "desc": self.data[branch][segment][metric_type][metric][
                                    "desc"
                                ],
                                "style": f"background:{row_background}; "
                                f"border-bottom-style: solid; "
                                f"border-right-style: solid;",
                    ...
                    for branch in self.data["branches"]:
                        ...
                        mean = "{0:.1f}".format(
                            self.data[branch][segment][metric_type][metric]["mean"]
                        )
                        std = "{0:.1f}".format(
                            self.data[branch][segment][metric_type][metric]["std"]
                        )

                        branch_mean = self.data[branch][segment][metric_type][metric][
                            "mean"
                        ]
                        control_mean = self.data[control][segment][metric_type][metric][
                            "mean"
                        ]
                        ...
                        pval = self.data[branch][segment][metric_type][metric]["tests"][
                            "mwu"
                        ]["p-value"]
                        effect_size = self.data[branch][segment][metric_type][metric][
                            "tests"
                        ]["mwu"]["effect"]
                    ...
                    numerical_metrics.append(
                        {
                            ...
                            "style": f"background:{row_background}; "
                            f"border-bottom-style:solid; border-right-style:solid;",
                            "datasets": datasets,
    ...
    def createCDFComparison(self, segment, metric, metric_type):
        ...
        control = self.data["branches"][0]
        values_control = self.data[control][segment][metric_type][metric]["pdf"][
            "values"
        ]
    ...
    def calculate_uplift_interp(self, quantiles, branch, segment, metric_type, metric):
        ...
        quantiles_control = self.data[control][segment][metric_type][metric][
            "quantiles"
        ]
        values_control = self.data[control][segment][metric_type][metric][
            "quantile_vals"
        ]
    ...
    def createUpliftComparison(self, segment, metric, metric_type):
        ...
        for branch in self.data["branches"]:
            ...
            [diff, uplift] = self.calculate_uplift_interp(
                quantiles, branch, segment, metric_type, metric
            )
            dataset = {
                "branch": branch,
                "diff": diff,
        ...
        for branch in self.data["branches"]:
            ...
            if branch != control:
                for test in self.data[branch][segment][metric_type][metric]["tests"]:
                    effect = "{0:.2f}".format(
                        self.data[branch][segment][metric_type][metric]["tests"][test][
                            "effect"
                        ]
                    )
                    pval = "{0:.2g}".format(
                    ...
                    dataset[test] = {"effect": effect, "pval": pval}
    ...
    def createPageloadEventMetrics(self, segment):
        for metric in self.data["pageload_event_metrics"]:
            with self.doc.div(id=f"{segment}-{metric}", klass="cell"):
                ...
                self.createMetrics(
                    segment, metric, "pageload_event_metrics", "numerical"
                )
